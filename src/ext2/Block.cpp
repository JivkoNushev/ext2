...
uint32_t Block::read(const char* file_name)
{
    ...
    if (!fields_buffer)
    {
        ...
        throw std::runtime_error("Null buffer in Block::read from derived class");
    }
    if (0 == this->m_size)
    {
        std::cerr << "[Error] Block size is 0\n";
        throw std::runtime_error("Block size is 0 in Block::read");
    }
    ...
    if (!ifs.is_open())
    {
        ...
        throw std::runtime_error("Failed to open device for Block::read");
    }
    ...
    if (ifs.fail())
    {
        ...
        ifs.close();
        throw std::runtime_error("Failed to seek in Block::read");
    }
    ...
    if (ifs.fail())
    {
        ...
        ifs.close();
        throw std::runtime_error("Failed to read data or incorrect amount in Block::read");
    }
...
uint32_t Block::write(const char* file_name) const
{
    ...
    if (!fields_buffer)
    {
        ...
        throw std::runtime_error("Null buffer in Block::write from derived class");
    }
    if (0 == this->m_size) 
    {
        std::cerr << "[Error] Block size is 0\n";
        throw std::runtime_error("Block size is 0 in Block::write");
    }
    ...
    {
        std::cerr << "[Error] Failed to open device " << file_name << '\n';
        throw std::runtime_error("Failed to open device for Block::write");
    }
    ...
    if (fs.fail())
    {
        std::cerr << "[Error] Failed to seek to offset " << this->m_offset << '\n';
        fs.close();
        throw std::runtime_error("Failed to seek in Block::write");
    }
    ...
    if (fs.fail())
    {
        ...
        fs.close();
        throw std::runtime_error("Failed to write data in Block::write");
    }
